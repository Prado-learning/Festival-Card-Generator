...
DEMO_LIST = [
  {
     "card": {
            ...
            "recipient": "妈妈",
            "nickname": "亲爱的妈妈",
            "input": "新春到，福气到！愿您岁岁平安如意，日日笑口常开！",  # 关键修改：字段名改为input
            "style": "中国风",
            "image_elements": "蛇年，红色背景, 金色福字, 鞭炮元素, 传统窗花"
        },
    "title": "春节🧧",
    "description": "给长辈的传统春节祝福"
  },
  {
     "card": {
            "index": 1,
            ...
            "recipient": "女朋友",
            "nickname": "我的宝贝",
            "input": "你是我眼中最亮的星，情人节快乐！",  # 字段名与组件变量一致
            "style": "浪漫星空",
            "image_elements": "星空背景, 爱心元素, 玫瑰花瓣"
        },
    "title": "情人节💝",
  ...
  },
  {
    "card": {
            ...
            "recipient": "朋友",
            "nickname": "老李",
            "input": "圣诞快乐！愿平安喜乐常伴，幸福美满永随！",
            "style": "Q版萌系",
            "image_elements": "圣诞树, 雪花, 礼物盒, 麋鹿元素"
        },
    "title": "圣诞节🎄",
    "description": "给朋友的圣诞祝福"
  },
  {
    "card": {
            "index": 3,
            ...
            "recipient": "家人",
            "nickname": "姐姐",
            "input": "生日快乐！愿时光静好，幸福常驻心间！",
            "style": "手绘插画",
            "image_elements": "24岁，蛇年，生日蛋糕, 彩色气球, 星星灯串, 礼物堆"
        },
    "title": "生日🎂",
    "description": "给家人的生日祝福"
  }
]
...
def demo_card_click(e: gr.EventData):
    try:
        ...
        # 数据校验
        assert card_data["festival"] in FESTIVAL_THEMES, f"无效节日：{card_data['festival']}"
        assert card_data["recipient"] in RECIPIENTS, f"无效关系：{card_data['recipient']}"
        
        # 打印调试信息
        print(f"[DEBUG] 加载示例数据：{card_data}")
        ...
        return [
            ...
            card_data["input"],
            card_data["style"],
            card_data["image_elements"]
        ]
    except Exception as e:
        print(f"【严重错误】示例加载失败：{str(e)}")
        return [gr.skip()]*6  # 跳过错误数据
                    ...
                    with ms.Div(elem_classes="example-container"):
                        with antd.Flex(gap="small", wrap=True):
                            with ms.Each(DEMO_LIST):
                                ...
                                demoCard.click(
                                    demo_card_click,
                                    outputs=[festival, recipient, nickname, input, style, image_elements]  # 第4个输出对应input
                                )
